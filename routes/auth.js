...
const JWT_SECRET = "WatchyourBa$ck";
...
router.post(
  ...
  async (req, res) => {
    // check if there are errors return the Bad request and the errors
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // check if password does not match with the password in database
      if (!passwordCompare) {
        return res
          .status(400)
          .json({ error: "Please try to login with correct credentials" });
